...
set cl_base=
set base_obj=
if %opt_pch%==1 set cl_base=call cl /c /TP /Zc:__cplusplus %cflags% /DBASE_IMPLEMENTATION base.h /nologo
if %opt_pch%==1 (
...
set cl_linalg=
set linalg_obj=
if %opt_pch%==1 set cl_linalg=call cl /c /TP /Zc:__cplusplus %cflags% /DLINALG_IMPLEMENTATION linalg.h /nologo
if %opt_pch%==1 (
...
set cl_gfx=
set gfx_obj=
if %opt_pch%==1 set cl_gfx=call cl /c /TP /Zc:__cplusplus %cflags% /DGFX_IMPLEMENTATION %dir_includes% gfx.h /nologo
if %opt_pch%==1 (
...
set cl_test=call cl /Zc:__cplusplus %cflags% %obj_flags% %dir_includes% test\test_all.cpp /link %lflags% %dir_libs% %libs% /nologo
